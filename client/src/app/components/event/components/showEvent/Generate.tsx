...
const Generate = ({ disabled, generateNow, event }: GeneratePropsType) => {
    ...
    return (
        <Box mt={2} display={'flex'} justifyContent={'center'} alignItems={'center'} flexDirection={'column'}>
            ...
            {
                (event.category?.category === "ELIMINATION" || event.category?.category === "SWISS") && <Typography color={!isPowerTwo(event.teams?.length!) ? '#cc3333' : '#33cc33'} variant="h6">The number of teams must be exact power of 2 (like 2, 4, 8, 16, 32 etc.)</Typography>
            }
            <Button size="large" variant="contained" color="success" sx={{ mt: 2 }} onClick={generateNow}
            disabled={disabled || ((event.category?.category === "ELIMINATION" || event.category?.category === "SWISS") && !isPowerTwo(event.teams?.length!))}>GENERATE NOW</Button>
        </Box>
    )
}
