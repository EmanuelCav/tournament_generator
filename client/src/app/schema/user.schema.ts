...
export const registerSchema = object().shape({
    fullname: string().trim().min(3, "Fullname field must has more than 2 characters")
    .matches(/^([\w]{3,})+\s+([\w\s]{3,})+$/i, "Please try to use letters to complete your fullname"),
    nickname: string().trim().min(3, "Nickname field must has more than 2 characters")
    ...
    confirm: string().min(1, 'Confirm password field is empty. Please complete').oneOf([ref('password')], 'Passwords do not match')
})